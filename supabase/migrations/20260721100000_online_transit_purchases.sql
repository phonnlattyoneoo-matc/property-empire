...
create or replace function public.online_transit_price(space_position integer)
returns integer
language sql
immutable
...
create or replace function public.online_transit_rent(station_count integer)
returns integer
language sql
immutable
set search_path = public
as $$
  select case
    ...
    when station_count = 1 then 25
    when station_count = 2 then 50
    else 100
  end;
$$;

create or replace function public.online_owned_transit_count(
...
as $$
  select
    case when coalesce(property_owners, '{}'::jsonb) ->> '4' = owner_id then 1 else 0 end +
    case when coalesce(property_owners, '{}'::jsonb) ->> '9' = owner_id then 1 else 0 end +
    case when coalesce(property_owners, '{}'::jsonb) ->> '15' = owner_id then 1 else 0 end +
    case when coalesce(property_owners, '{}'::jsonb) ->> '21' = owner_id then 1 else 0 end;
$$;
...
begin
  ...
  if property_position is null then
    raise exception 'Only property and transit spaces can be skipped.'
      using errcode = '22023';
  end if;

  purchase_price := coalesce(
    public.online_property_price(property_position),
    public.online_transit_price(property_position)
  );
...
revoke all on function public.online_transit_price(integer) from public;
revoke all on function public.online_transit_rent(integer) from public;
revoke all on function public.online_owned_transit_count(jsonb, text) from public;
